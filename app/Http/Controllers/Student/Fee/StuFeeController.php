...
class StuFeeController extends Controller
{
    public function StudentFeeView(){
        ...
        return view('admin.student.fee.stu_fee',compact('student_fee'));
    }
    ...
    public function StudentFeeAdd(){
        ...
        return view('admin.student.fee.add_stu_fee',compact('years','classes','fee_categories'));
    }

    public function StudentFeeSearch(Request $request){
     ...
     $registrationfee = FeeAmount::where('fee_id',$fee_category_id)->where('class_id',$std->class_id)->first();
     ...
     if($accountstudentfees !=null) {
        ...
        public function submitOtherFee(){
            $other_fee_submit = OtherFeeSubmit::with('student','year','class')->latest()->get();
            return view('admin.student.other_fee.submit.other_fee_submit',compact('other_fee_submit'));
        }

        public function submitOtherFeeAdd(){
            $years = StudentYear::all();
            $classes = StudentClass::all();
            $other_fee = OtherFee::all();
            return view('admin.student.other_fee.submit.add_other_fee_submit',compact('years','classes','other_fee'));
        }

        public function submitOtherFeeSearch(Request $request){
            $year_id = $request->year_id;
            $class_id = $request->class_id;
            $date = date('Y-m',strtotime($request->date));

            $data = AssignStudent::with(['discount','student'])->where('year_id',$year_id)->where('class_id',$class_id)->get();
            ...
            foreach ($data as $key => $std) {
                $otherFee = OtherFee::where('year_id',$std->year_id)->where('class_id',$std->class_id)->get();

                $accountstudentfees = OtherFeeSubmit::where('student_id',$std->student_id)->where('year_id',$std->year_id)->where('class_id',$std->class_id)->where('date',$date)->first();
                ...
                $html[$key]['tdsource']  .= '<td>'.$otherFee->sum('fee_amount').'BDT'.'<input type="hidden" name="date" value= " '.$date.' " >'.'</td>';

                $html[$key]['tdsource'] .= '<td>'.$std['discount']['discount'].'%'.'</td>';

                $orginalfee = $otherFee->sum('fee_amount');
                $discount = $std['discount']['discount'];
                $discountablefee = $discount/100*$orginalfee;
                $finalfee = (int)$orginalfee-(int)$discountablefee;
        ...
        public function submitAccountFeeStore(Request $request){
            $date = date('Y-m',strtotime($request->date));

            OtherFeeSubmit::where('year_id',$request->year_id)->where('class_id',$request->class_id)->where('date',$request->date)->delete();
            $checkdata = $request->checkmanage;

            if ($checkdata !=null) {
                for ($i=0; $i <count($checkdata) ; $i++) {
                    $data = new OtherFeeSubmit();
                    $data->year_id = $request->year_id;
                    $data->class_id = $request->class_id;
            ...
            if (!empty(@$data) || empty($checkdata)) {
                ...
                return redirect()->route('submit.other.fee')->with($notification);
